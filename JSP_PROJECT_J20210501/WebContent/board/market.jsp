...
<body>
	<div id="wrapper">
		<section id="section">
			<div id="post_title">
				<h1><a href="market.do">장터 게시판</a></h1>
			</div>
			<table class="post_list">
				...
				<c:if test="${startPage > blockSize }">
					<a href="market.do?pageNum=${startPage - blockSize }">[이전]</a>
				</c:if>
				<c:forEach var="i" begin="${startPage }" end="${endPage }">
					<a href="market.do?pageNum=${i }">${i }</a>
				</c:forEach>
				<c:if test="${endPage < pageCnt }">
					<a href="market.do?pageNum=${startPage + blockSize }">[다음]</a>
				</c:if>
				</td></tr>
				</table>
				<a href="writeForm.do?board_no=2"><input type="button" value="글쓰기" id="write_but"></a>
		</section>
	</div>
</body>
</html>
